...
// 族群：⚔️＝今天平均低於昨收（平均漲跌幅 < 0）、🔪＝平均低於前天收盤（兩個各自看，都成立就都顯示）。
...
// 32 刀劍空：馬火多的鏡像。今天下跌、不比櫃買強、族內後 1/3；⚔️＝現價低於昨收、🔪＝現價低於前天收盤（數字＝低幾 %），
//   都沒有就 〰️。門檻跟馬火多對稱（2026-09-23 使用者：名單太長）：今天跌 BLADE_MIN_PCT%～BLADE_MAX_PCT%、不含跌停
//   鎖死、族內後 1/3、成交量 ≥ BLADE_MIN_VOLUME 張、而且要 🔪（現價低於前天收盤，兩天都弱才算）。
...
function race333Model(){
  ...
  const info = ranked.map((g) => {
    ...
    const weakerThanYesterday = g.avgChange < 0;                          // ⚔️ 平均低於昨收
    const weakerThanDayBefore = twoDayAvg !== null && twoDayAvg < 0;      // 🔪 平均低於前天收盤
    return {
  ...
  groups.forEach((g) => {
    ...
    valid.forEach((s, i) => {
      ...
      const row = {
        ...
        sword: s.changePercent < 0,                               // ⚔️ 現價低於昨收
        knife: twoDay !== null && twoDay < 0 ? -twoDay : null,    // 🔪 現價低於前天收盤，數字＝低幾 %
      };
  ...
  // 32 刀劍空：今天跌 BLADE_MIN_PCT% 以上（且 ≤ 櫃買%）、族內後 1/3、成交量夠、現價低於前天收盤（🔪）；
  ...
  const bladeLive = bladeBase.filter((r) => !bladeHarvest.includes(r) && r.knife !== null);
...
function bladeBadge(r){
  const marks = (r.sword ? '⚔️' : '') + (r.knife !== null ? '🔪' + r.knife.toFixed(1) : '');
...
function raceGroupRowHtml(g){
  ...
  const marks = (g.weakerThanDayBefore ? '🔪' : '') + (g.weakerThanYesterday ? '⚔️' : '');
...
function groupHolderForceModel(){
  ...
  const cardFor = (g, mode) => {
    ...
    const rows = valid.map((s) => {
      ...
      if (mode === 'up' ? !(h.strengthPct > 0) : !(h.strengthPct < 0)) return null;
      return Object.assign({}, h, {
        pct: s.changePercent, groupName: g.name, groupRank: rankOf.get(g.name),
...
function groupHolderForceWarnPillHtml(r){
  // 使用者要求：超過漲跌幅鎖死(漲停/跌停)、或會被處置(關)的個股，紅底白字寫在該列最後面。
  const parts = [];
  if (r.limitUp) parts.push('漲停');
  if (r.limitDown) parts.push('跌停');
  const f = stockFlags[r.code];
  if (f && f.disposition){
    const until = f.dispositionUntil ? ' 至' + String(f.dispositionUntil).slice(5).replace('-', '/') : '';
    parts.push('處置股' + until);
  }
  if (!parts.length) return '';
  const title = f && f.disposition && f.dispositionReason ? f.dispositionReason : '';
  return '<span class="pill-warn"' + (title ? ' title="' + title + '"' : '') + '>' + parts.join('・') + '</span>';
}
function groupHolderForceStockRowHtml(r, idx){
  const backendName = r.name && r.name !== r.code ? r.name : lookupStockName(r.code);
  const cls = r.strengthPct > 0 ? 'sig-bull' : 'sig-bear';
  const pctText = (r.strengthPct > 0 ? '+' : '') + r.strengthPct.toFixed(1) + '%';
  ...
  return '<div class="race-row stock-row" data-code="' + r.code + '" data-name="' + backendName + '" tabindex="0" role="button">' +
    ...
    groupHolderForceWarnPillHtml(r) + '</div>';
}
...
function groupHolderForceHtml(){
  ...
  if (!m) return '<div class="signal-empty"><div class="se-title">族群行情還沒載入</div><div class="se-sub">首頁資料抓到後就會出現。</div></div>';
  const stamp = new Date().toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit', hour12: false });
  return '<div class="race-sub">今天漲幅前10大族群，各取大戶力（大單淨額÷累計成交額）最強的前5名個股；跌幅前10大族群，各取大戶力最負(賣超)的前5名個股。族群依今天平均漲跌幅排名，共 ' + m.total + ' 個族群。' + stamp + '</div>' +
    '<div class="race-sep">------↑(漲幅前10大族群・大戶力Top5)↑------</div>' +
    m.risingGroups.map((c) => groupHolderForceCardHtml(c, 'up')).join('') +
    '<div class="race-sep">------↓(跌幅前10大族群・大戶力Top5)↓------</div>' +
...
function groupCombinedBoardModel(){
  ...
  const blocks = ranked.map((g) => {
    ...
    const rows = valid.map((s) => {
      ...
      if (!h && !hasDisp) return null;
      return {
        code: s.code, name: s.name, pct: s.changePercent,
...
function groupCombinedBoardRowHtml(r){
  ...
  const lock = r.limitUp ? '🔒漲停' : r.limitDown ? '🔒跌停' : '';
  return '<tr class="combo-row stock-row" data-code="' + r.code + '" data-name="' + backendName + '" tabindex="0" role="button">' +
    '<td class="combo-code">' + r.code + '</td>' +
    '<td class="combo-name">' + backendName + '</td>' +
    '<td class="combo-pct ' + dirClass(r.pct) + '">' + fmt(r.pct) + '%' + lock + '</td>' +
...
function groupCombinedBoardBlockHtml(block){
  return '<div class="race-block"><div class="race-head">' + block.name + '（' + block.groupTotal + ' 檔） 今天平均 ' + fmt(block.avgChange) + '%・有大戶力或處置/注意資料 ' + block.rows.length + ' 檔</div>' +
...
function groupCombinedBoardHtml(){
  ...
  const stamp = new Date().toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit', hour12: false });
  return '<div class="race-sub">大戶力（大單淨額÷累計成交額）跟處置/注意狀態合併顯示，一個族群一個表格；只列出有大戶力資料或有處置/注意資料的股票。族群依今天平均漲跌幅排序，共 ' + m.blocks.length + ' 個族群、' + m.total + ' 檔。' + stamp + '</div>' +
