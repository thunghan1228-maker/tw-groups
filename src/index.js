...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .srow-right{display:flex;flex-direction:column;align-items:flex-end;gap:1px;flex-shrink:0;}
  .srow-right .sprice{font-size:13px;font-weight:800;font-variant-numeric:tabular-nums;}
  .srow-right .schg{font-size:11px;font-weight:700;font-variant-numeric:tabular-nums;}
  ...
  #klineCanvas,#volumeCanvas,#forceCanvas,#macdCanvas{display:block;width:100%;height:100%;touch-action:none;}
  ...
  /* 盤中訊號中心 */
  .signal-modal{position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:101;}
  .signal-modal-inner{position:absolute;background:var(--bg);border:1px solid var(--line);border-radius:14px;width:min(560px,94vw);height:min(640px,86vh);min-width:320px;min-height:320px;max-width:98vw;max-height:96vh;overflow:auto;padding:14px;box-shadow:0 12px 40px rgba(0,0,0,0.5);}
...
<div class="draft-badge"><span class="dot"></span>族群報價／櫃買盤勢／盤中訊號中心（除四項精選外）為後端真實資料，連不上時自動退回示範資料；K線圖／主力買賣力／日線、四項精選目前仍為示範資料</div>
<div class="stock-search-bar">
  <input type="text" id="stockSearchInput" placeholder="輸入代號或名稱，按 Enter 開啟五分鐘K線圖">
  ...
  <div class="signal-modal-inner" id="signalModalInner">
    <div class="signal-modal-head" id="signalModalHead">
      ...
      <div class="sm-actions">
        ...
        <button class="cm-icon-btn" id="smClose" aria-label="關閉">✕</button>
      </div>
    </div>
    <div class="signal-help" id="signalHelp" hidden>
      <div>今日即時：彙整下列各類訊號的即時清單。</div>
      <div>族群瞬間大單：同族群內短時間出現的大額單筆成交。</div>
      <div>主力累計：個股主力（大戶）今日累計買超／賣超達門檻。</div>
      <div>四項精選：同時符合多項篩選條件的精選個股。</div>
...
const TF_CONFIG = {
  daily: { label: '日線', maPeriods: [5, 20, 60, 120, 240], bars: 180, vwap: false, force: false },
...
let currentChart = {
  code: null, name: null, tf: 'daily', bars: null, force: null, macd: null,
...
function dailyBarToBar(b){
  ...
  return {
    ts: b.ts, open: b.open, high: b.high, low: b.low, close: b.close,
    volume: b.volume ?? 0, label, fullLabel, mainNet: null,
  };
}
...
function openGroupDetail(groupName){
  ...
  document.getElementById('gmList').innerHTML = sorted.map((s) => {
    ...
    return '<div class="stock-row" data-code="' + s.code + '" data-name="' + s.name + '" tabindex="0" role="button">' +
      ...
      '<div class="srow-right"><span class="sprice">' + price.toFixed(2) + '</span>' +
      '<span class="schg ' + dirClass(s.changePercent) + '">' + (changeAmt > 0 ? '+' : '') + changeAmt.toFixed(2) + ' (' + fmt(s.changePercent) + '%)</span></div>' +
    '</div>';
  }).join('');
...
function initWindowChrome(){
  ...
  head.addEventListener('pointerdown', (e) => {
    if (e.target.closest('.cm-icon-btn')) return;
...
async function renderOtcStrengthWidget(){
  ...
  try {
    ...
    const data = await res.json();
    if (!data || !data.ready) throw new Error('otc-strength not ready');
...
function buildDemoSignalsForDate(dateStr){
  ...
  const counts = { groupBigOrder: 3, mainForceCum: 2, fourGate: 1, bigBuy: 2, bigSell: 2, bigHolderForce: 2 };
  const events = [];
  let si = 0;
  Object.keys(counts).forEach((kind) => {
    for (let i = 0; i < counts[kind]; i++){
      ...
      else if (kind === 'mainForceCum') label = '主力累計' + (rnd() > 0.5 ? '買超 ' : '賣超 ') + qty + ' 張';
...
function mapLargeOrderSignal(s){
  const isBuy = s.kind === 'instantLargeBuy';
  return {
...
async function fetchMainForceRanking(){
  const res = await fetch('/api/main-force-ranking?limit=30');
...
function renderSignalCenter(){
  ...
  const countFor = (key) => {
    ...
    if (key === 'history' || key === 'fourGate') return null;
    if (key === 'mainForceCum') return mainForceRanking.length;
  ...
  } else if (active === 'mainForceCum'){
    body.innerHTML = rankingRowsHtml(mainForceRanking);
  } else if (active === 'bigHolderForce'){
    body.innerHTML = rankingRowsHtml(bigHolderRows);
  } else if (active === 'fourGate'){
    body.innerHTML = '<div class="signal-empty"><div class="se-title">四項精選規則待確認</div>' +
      '<div class="se-sub">這個分類要同時符合哪4項條件還在確認，先保留頁籤位置，之後補上。</div></div>';
  } else {
...
function positionSignalModal(){
  ...
  const h = Math.min(640, window.innerHeight * 0.86);
  panel.style.width = w + 'px'; panel.style.height = h + 'px';
  panel.style.left = Math.max(0, (window.innerWidth - w) / 2) + 'px';
  panel.style.top = Math.max(10, (window.innerHeight - h) / 2) + 'px';
}
...
function openSignalCenter(){
  ...
  overlay.hidden = false;
  positionSignalModal();
  if (!document.body.classList.contains('signal-window-mode')) document.body.style.overflow = 'hidden';
  renderSignalCenter();
}
function hideSignalModalPanel(){
  document.getElementById('signalModal').hidden = true;
  if (!document.body.classList.contains('signal-window-mode')) document.body.style.overflow = '';
}
...
if (document.body.classList.contains('signal-window-mode')) openSignalCenter();
