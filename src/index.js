...
.bl-tag{display:inline-block;font-size:10px;font-weight:700;border-radius:6px;padding:0 6px;margin-left:4px;background:#7c3aed;color:#fff;white-space:nowrap;}
  ...
  .combo-filter-bar .combo-filter-btn,.combo-filter-bar .hf-filter-btn,.combo-filter-bar .hf-day-btn,.combo-filter-bar .race333-filter-btn,.combo-filter-bar .bigorder-filter-btn,.combo-filter-bar .bl-day-btn,.combo-filter-bar .sig-day-btn{padding:4px 10px;font-size:12px;}
  ...
  .hf-filter-btn,.combo-filter-btn,.race333-filter-btn,.bigorder-filter-btn,.bl-day-btn,.sig-day-btn{background:#000;color:#fff;border-color:#000;}
  .combo-filter-bar .bl-day-btn[disabled],.combo-filter-bar .sig-day-btn[disabled]{opacity:.45;cursor:default;}
  .hf-filter-btn:hover,.combo-filter-btn:hover,.race333-filter-btn:hover,.bigorder-filter-btn:hover{background:#2a2a2a;}
  .hf-filter-btn.active,.combo-filter-btn.active,.race333-filter-btn.active,.bigorder-filter-btn.active,.bl-day-btn.active,.sig-day-btn.active{background:#000;color:#c084fc;border-color:#000;font-weight:800;}  /* 按下去整個字變紫色（2026-09-24 使用者），不用外框 */
  ...
  <div class="signal-modal-inner" id="signalModalInner">
    ...
    <div class="signal-help" id="signalHelp" hidden>
      <ul>
        ...
        <li><b>族群綜合表</b>：大戶力（大單淨額÷累計成交額）跟處置/注意狀態合併顯示，一個族群一個表格；只列出大戶力≥+10%或≤-10%、或有處置/注意資料的股票。處置／注意欄：「處置中」是官方處置股名單，藍色「注意股」是交易所已公布的官方注意股（都是官方現成資料）。</li>
...
function signalRowsHtml(events, view){
  ...
  return '<div class="signal-list">' + events.map((ev) => signalEventRowHtml(ev, view)).join('') + '</div>';
}
...
function brewTodayRows(){
  ...
  const launch = new Map(recorded.launch.map((r) => [r.code, r]));
