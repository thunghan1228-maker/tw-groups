...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .signal-modal-inner{position:absolute;background:var(--bg);border:1px solid var(--line);border-radius:14px;width:min(560px,94vw);height:min(640px,86vh);min-width:280px;min-height:320px;max-width:98vw;max-height:96vh;overflow:auto;resize:both;padding:14px;box-shadow:0 12px 40px rgba(0,0,0,0.5);pointer-events:auto;}
...
<div class="draft-badge"><span class="dot"></span>族群報價／櫃買盤勢／盤中訊號中心（含四項精選）為後端真實資料，連不上時自動退回示範資料；K線圖／主力買賣力／日線目前仍為示範資料</div>
...
<div class="signal-modal" id="signalModal" hidden>
  <div class="signal-modal-inner" id="signalModalInner">
    ...
    <div class="signal-help" id="signalHelp" hidden>
      ...
      <div>四項精選（強多/強空）：主力淨額比、價格位置兩項條件同時成立才會出現。強多＝當分鐘主力淨買超≥50%且前一分鐘同方向，且價格同時站上VWAP、突破開盤前5分鐘（09:00-09:04）K棒高點；強空＝主力淨賣超≥50%且前一分鐘同方向，且價格同時跌破VWAP、跌破開盤前5分鐘K棒低點。同一檔股票同一方向一天只提示一次，偵測時間09:00-13:30。</div>
...
function mapLargeOrderSignal(s){
  ...
  return {
    ...
    code: s.ticker, name: backendName || lookupStockName(s.ticker), group: s.groupName, label: s.label,
  };
}
