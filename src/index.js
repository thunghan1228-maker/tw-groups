...
.bl-tag{display:inline-block;font-size:10px;font-weight:700;border-radius:6px;padding:0 6px;margin-left:4px;background:#7c3aed;color:#fff;white-space:nowrap;}
  ...
  @media (max-width: 640px){
    ...
    .tb-btn{min-width:64px;}  /* 2026-09-26：六顆在 390px 的手機也要一排放得下（6×64=384） */
  ...
  <div class="chips-inner" id="chipsInner">
    <div class="chips-head">
      <div>
        ...
        <div class="chips-sub">主力大單（永豐逐筆算的大單淨額）＋三大法人買賣超（證交所、櫃買中心公開資料），每個交易日收盤後更新；只列 43 個族群的股票，點股票可開K線圖。</div>
      </div>
...
<div class="toolbar-bottom" id="toolbarBottom">
  ...
  <!-- 2026-09-26 使用者：下方導覽列的「三角收斂」「隔日沖大單籌碼」不用再留著（本來就是開發中的空位），拿掉後手機一排放得下六顆。 -->
...
function closeStockChart(){
  ...
  // 若是從訊號中心／盤後籌碼排行點進來的，關圖後讓它們回到最上層。
  document.getElementById('signalModal').classList.remove('behind-chart');
  document.getElementById('chipsModal').classList.remove('behind-chart');
  document.getElementById('swingModal').classList.remove('behind-chart');
}
...
const SIGNAL_KINDS = [
  ...
  { key: 'brewLaunch', label: '醞釀／發動' },   // 2026-09-26 使用者：跟盤中大戶力對調
...
const SWING_SECTIONS = [
  { key: 'summary', label: '今日摘要' }, { key: 'disposition', label: '處置動態' }, { key: 'groups', label: '產業觀察' },
  { key: 'chips', label: '籌碼面' }, { key: 'tech', label: '技術面' }, { key: 'ma', label: '均線轉強' },
];
...
function swingSectionHtml(data){
  ...
  if (sec === 'summary'){
    ...
    return '<div class="sw-tiles">' +
      ...
      '<div class="sw-tile"><b>' + (t.maJump || 0) + '</b><span>均線轉強(檔)</span></div>' +
      ...
      '<div class="sw-tile"><b>' + (t.releasing || 0) + '</b><span>明日出獄(檔)</span></div></div>' +
...
const SWING_FOOT = '<div class="sw-foot">教學與觀念紀錄，依系統既有資料整理，非投資建議；個股僅為型態與數據紀錄，非買賣推介。本益比、月營收年增、集保 400 張以上大戶週籌碼與處置動態每個交易日收盤後接入；上一週籌碼對照與主動式 ETF 五檔是下一階段要接的資料。</div>';
