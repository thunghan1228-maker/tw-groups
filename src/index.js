...
function brewOverlapHtml(mode){
  ...
  const later = mode === 'x01'
    ? { label: '今天', date: brewSessionDate(), rows: brewTodayRows() }
    : { label: '昨天 ' + mmdd(past[0]), date: past[0], rows: brewHistoryDay(past[0]) };
  const earlier = mode === 'x01'
    ? { label: '昨天 ' + mmdd(past[0]), date: past[0], rows: brewHistoryDay(past[0]) }
    : { label: '前天 ' + mmdd(past[1]), date: past[1], rows: brewHistoryDay(past[1]) };
  ...
  const laterName = later.label.slice(0, 2), earlierName = earlier.label.slice(0, 2);
